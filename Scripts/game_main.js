...
game_table.addEventListener('click', function(event) {
    ...
    if(event.target.hasAttribute('data-question')) {
        ...
        let point_value = clicked_box.innerText.replace('$', '')
        point_value = parseFloat(point_value)
        
        _dom.default_point_value = point_value
        ...
        choices.addEventListener('click', function(event) {
            let correctAnswer = document.getElementById(`${clicked_question.id}-answer`).innerHTML
        ...
        question_timer.addEventListener("animationend", function() {
...
function getRandomOptionSlot(element, index, option, json) {
    ...
    do {
        if(pickedSlot.hasAttribute('data-choices', 'correct') || pickedSlot.hasAttribute('data-choices', 'wrong_1')) {
            ...
            pickedSlot.setAttribute('id', `${element.parentElement.id}-${option}`)
        }
    }
